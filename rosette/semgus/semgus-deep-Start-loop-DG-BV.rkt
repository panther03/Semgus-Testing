...
(define-symbolic x y c (bitvector 8))
...
(define (Start.Sem p x y c)
  (destruct p
    ...
    [(while_st cond stmt)   (if (&& (B.Sem cond x y) (bvugt c 0))
                                (let ([stmt_sem (Start.Sem stmt x y c)])
                                  (Start.Sem p (first stmt_sem) (second stmt_sem) (bvsub c 1)))
                                (list x y c))]
...
(define (E.Sem p x y)
  (destruct p
    ...
    [(sub_st iexp1 iexp2) (bvsub (E.Sem iexp1 x y) (E.Sem iexp2 x y))]
    [(varx_st) x]
    [(vary_st) y]
    [(const1_st) (bv #b1 8)]
    [(const0_st) (bv #b0 8)]
...
(define (B.Sem p x y)
  (destruct p
    [(lt_st iexp1 iexp2) (bvult (E.Sem iexp1 x y) (E.Sem iexp2 x y))]
    [(gt_st iexp1 iexp2) (bvugt (E.Sem iexp1 x y) (E.Sem iexp2 x y))]
    [(true_st) (bv #b1 8)]
    [(false_st) (bv #b0 8)]
    [(not_st bexp) (bvnot (B.Sem bexp x y))]
    [(and_st bexp1 bexp2) (bvand (B.Sem bexp1 x y) (B.Sem bexp2 x y))]
    [_ p]))
...
(define sol_start
  (synthesize
   #:forall (list)
   #:guarantee  (assert (let ([test (Start.Sem (f) (bv 0 8) (bv 0 8) (bv 8 8))]) (&& (equal? (first test) (bv 0 8))  (equal? (second test) (bv 0 8)))))
                            ))

; Test constraints
;(equal? (Start.Sem max2_start (bv 0 8) 7) (list 4 3))
